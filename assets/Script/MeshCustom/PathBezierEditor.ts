...
export default class RouteEditorCurveBezier extends cc.Component {
    ...
    @property(cc.Sprite) spMesh: cc.Sprite = null;
    ...
    private viewCurve(collider: cc.PolygonCollider) {
        ...
        /** 2.1.2 wrapper */
        let pWorld = this.node.convertToWorldSpaceAR(cc.Vec2.ZERO);
        p1 = p1.add(pWorld);
        p2 = p2.add(pWorld);
        p3 = p3.add(pWorld);
        p4 = p4.add(pWorld);
    ...
    private viewBorder() {
        ...
        for(let i = 0; i <= this.splitCount; i++) {
            ...
            if (p0) {
                ...
                let pv0 = cc.v2(-dir.y, dir.x).mul(this.borderWidth);
                let pv1 = cc.v2(dir.y, -dir.x).mul(this.borderWidth);
        ...
        if (this.spMesh) {
            let tw = this.spMesh.spriteFrame.getTexture().width;
            let th = this.spMesh.spriteFrame.getTexture().height;
            let ow = this.spMesh.node.width;
            let oh = this.spMesh.node.height;
            ...
            this.spMesh.spriteFrame.vertices = {
                x: x, // 左上 左下 右上 右下
                y: y,
                nu: nu,
                nv: nv,
            ...
            this.spMesh.markForUpdateRenderData(true);
        }
    }
    ...
    private viewDistance(step: number) {
        ...
        /** 2.1.2 wrapper */
        let pWorld = this.node.convertToWorldSpaceAR(cc.Vec2.ZERO);
        posArr = posArr.map(ele => ele.add(pWorld));
